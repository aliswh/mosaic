...
# arguments 
model_tags=("gemma-12b" "llama-8b" "medgemma-4b")  

train_datasets=('mimic' 'padchest_EN' 'padchest' 'casia' 'danskcxr')
valid_datasets=('mimic' 'padchest_EN' 'padchest' 'casia' 'danskcxr')
test_datasets=('mimic' 'padchest' 'padchest_EN' 'casia' 'danskcxr' 'reflacx_i' 'reflacx_ii')
...
for model_tag in $model_tags; do
    if $train_flag; then
        python3 $train_path -m $model_tag -ct "mppecd" -p $wandb_project \
