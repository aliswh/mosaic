...
train_datasets=('mimic' 'padchest_EN' 'padchest')
valid_datasets=('mimic' 'padchest_EN' 'padchest')
test_datasets=('mimic' 'padchest' 'padchest_EN' 'casia' 'danskcxr')
...
for model_tag in $model_tags; do
    if $train_flag; then
        python3 $train_path -m $model_tag -ct "mppe" -p $wandb_project \
