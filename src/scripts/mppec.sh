...
infer_path=$RRC_WORK_DIR"/src/inferencem.py"

# arguments 
model_tags=("gemma-4b" "gemma-12b" "llama-8b")


train_datasets=('mimic' 'padchest_EN' 'padchest' 'casia')
valid_datasets=('mimic' 'padchest_EN' 'padchest' 'casia')
test_datasets=('danskcxr' 'mimic' 'padchest' 'padchest_EN' 'casia' ) 
...
for model_tag in $model_tags; do
    if $train_flag; then
        python3 $train_path -m $model_tag -ct "mppec" -p $wandb_project \
