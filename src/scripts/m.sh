...
# arguments 
model_tags=("llama-3b" "mmed-llama-8b" "llama-8b" "gemma-4b" "medgemma-4b" "gemma-12b") 


train_datasets=('mimic')
valid_datasets=('mimic')
...
for model_tag in $model_tags; do
    if $train_flag; then
        python3 $train_path -m $model_tag -ct "m" -p $wandb_project \
