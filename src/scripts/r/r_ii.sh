...
train_datasets=('reflacx_ii')
valid_datasets=('reflacx_ii')
test_datasets=('reflacx_ii')
...
for model_tag in $model_tags; do
    if $train_flag; then
        python3 $train_path -m $model_tag -ct "r_ii" -p $wandb_project \
            -tds "$train_datasets" -vds "$valid_datasets" \
            -c $checkpoint $RRC_WORK_DIR"/output/models/"$model_tag"_mimic-padchest_EN-padchest-casia-danskcxr"
            #-et $special_tag  
    fi
    if $test_flag; then
