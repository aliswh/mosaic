...
# arguments 
model_tags=("medgemma-4b" "llama-8b" "gemma-12b")


train_datasets=('mimic' 'padchest_EN')
valid_datasets=('mimic' 'padchest_EN')
...
for model_tag in $model_tags; do
    if $train_flag; then
        python3 $train_path -m $model_tag -ct "mpe" -p $wandb_project \
