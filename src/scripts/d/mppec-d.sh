...
train_datasets=('danskcxr')
valid_datasets=('danskcxr')
test_datasets=('danskcxr')
...
for model_tag in $model_tags; do
    if $train_flag; then
        python3 $train_path -m $model_tag -ct "mppec-d" -p $wandb_project \
            -tds "$train_datasets" -vds "$valid_datasets" \
            -c $checkpoint $RRC_WORK_DIR"/output/models/"$model_tag"_mimic-padchest_EN-padchest-casia"
            #-et $special_tag  
    fi
    if $test_flag; then
